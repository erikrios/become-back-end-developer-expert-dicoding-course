...
describe('HTTP server', () => {
  ...
  describe('when POST /users', () => {
    ...
    it('should response 400 when request payload not meeet data type specification', async () => {
      // Arrange
      const requestPayload = {
        username: 'dicoding',
        password: 'secret',
    ...
    it('should response 400 when username contain restricted character', async () => {
      ...
      expect(response.statusCode).toEqual(400);
      expect(responseJson.status).toEqual('fail');
      expect(responseJson.message).toEqual('tidak dapat membuat user baru karena username mengandung kareakter terlarang');
    });
